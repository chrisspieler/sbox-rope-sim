...
{ 
	...
	CreateTexture2DWithoutSampler( g_tBorderImage ) 	< Attribute( "BorderImageTexture" ); Default( 1.0 ); >;
	...
	float4 AddImageBorder( float2 texCoord )
	{
		...
		float4 r = AttributeTex2DS( g_tBorderImage, g_sClampSampler, uv );
	...
	PS_OUTPUT MainPs( PS_INPUT i )
	{
		...
		if ( D_BACKGROUND_IMAGE == 1 )
		{
			...
			if ( BgRepeat == 0 )
				vImage = AttributeTex2DS( tex, g_sRepeatSampler, vUV );
			else if ( BgRepeat == 1 )
				vImage = AttributeTex2DS( tex, g_sRepeatXSampler, vUV );
			else if ( BgRepeat == 2 )
				vImage = AttributeTex2DS( tex, g_sRepeatYSampler, vUV );
			else
				vImage = AttributeTex2DS( tex, g_sClampSampler, vUV );
